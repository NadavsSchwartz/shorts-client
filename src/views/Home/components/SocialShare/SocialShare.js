...
import { ReactComponent as FacebookSVG } from './components/Facebook.svg';
import { ReactComponent as TwitterSVG } from './components/Twitter.svg';
import { ReactComponent as WhatsAppSVG } from './components/WhatsApp.svg';
import { ReactComponent as TelegramSVG } from './components/Telegram.svg';
import { ReactComponent as RedditSVG } from './components/Reddit.svg';
import { ContentCopy } from '@mui/icons-material';
import DialogTitle from '@mui/material/DialogTitle';
import Dialog from '@mui/material/Dialog';
import { Alert, Grid, Snackbar, Typography } from '@mui/material';
import CustomSocialShare from 'components/CustomSocialShare/CustomSocialShare';
    ...
    <>
      ...
      <Grid container spacing={0.5} sx={{ cursor: 'pointer' }}>
        <Grid
          item
          xs
          onClick={() => CustomSocialShare(Link && Link.shortUrl, 'Facebook')}
        >
          <FacebookSVG />
        </Grid>
        <Grid
          item
          xs
          onClick={() => CustomSocialShare(Link && Link.shortUrl, 'Twitter')}
        >
          <TwitterSVG />
        </Grid>
        <Grid
          item
          xs
          onClick={() => CustomSocialShare(Link && Link.shortUrl, 'WhatsApp')}
        >
          <WhatsAppSVG />
        </Grid>
        <Grid
          item
          xs
          onClick={() => CustomSocialShare(Link && Link.shortUrl, 'Telegram')}
        >
          <TelegramSVG />
        </Grid>
        <Grid
          item
          xs
          onClick={() => CustomSocialShare(Link && Link.shortUrl, 'Reddit')}
        >
          <RedditSVG />
        </Grid>
        <Grid item xs>
        ...
        <Grid item xs onClick={handleChange}>
          <svg
